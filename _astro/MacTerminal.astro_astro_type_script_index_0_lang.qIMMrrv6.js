...
`;const a=s==="arm64"?"amd64":"arm64",l=e==="darwin"?a==="arm64"?"Apple Silicon":"Intel":a==="arm64"?"ARM64":"x86_64",d=t.assets[`${e}_${a}`]??"";d&&(o+=`Wrong arch? ${l}: ${d}`)}P(o)}let g=null,_=0,L=[];async function At(){let t=[];try{t=await(await fetch("/screenshots/index.json")).json()}catch{F("output-error","Could not load screenshots. Run tools/screenshots/take.sh first.");return}if(t.length===0){F("output-dim","No screenshots yet. Run: tools/screenshots/take.sh");return}L=t,_=0,g&&g.remove(),g=document.createElement("div"),g.className="output-line screenshot-gallery",g.setAttribute("tabindex","0"),$.appendChild(g),Y(),Z(),g.addEventListener("keydown",e=>{e.key==="ArrowRight"||e.key==="l"?(e.preventDefault(),_=(_+1)%L.length,Z()):e.key==="ArrowLeft"||e.key==="h"?(e.preventDefault(),_=(_-1+L.length)%L.length,Z()):(e.key==="Escape"||e.key==="q")&&(e.preventDefault(),Ct(),E.focus())}),g.focus()}function Z(){if(!g||L.length===0)return;const t=L[_];for(;g.firstChild;)g.removeChild(g.firstChild);const e=document.createElement("div");e.className="sg-header";const s=document.createElement("span");s.className="sg-counter",s.textContent=`[${_+1}/${L.length}]`;const n=document.createElement("span");n.className="sg-caption",n.textContent=t.caption;const r=document.createElement("span");r.className="sg-keys",r.textContent="← → navigate · esc close",e.appendChild(s),e.appendChild(n),e.appendChild(r);const i=document.createElement("div");i.className="sg-img-wrap";const c=document.createElement("img");c.className="sg-img",c.setAttribute("loading","lazy"),c.src=`/screenshots/${t.file.replace(/[^a-zA-Z0-9._-]/g,"")}`,c.alt=t.caption,i.appendChild(c);const o=document.createElement("div");o.className="sg-dots",L.forEach((u,a)=>{const l=document.createElement("span");l.className=a===_?"sg-dot sg-dot-active":"sg-dot",o.appendChild(l)}),g.appendChild(e),g.appendChild(i),g.appendChild(o)}function Ct(){g&&(g.remove(),g=null)}const pe=[{name:"code-review",desc:"local model reads the diff, cloud model judges it",yaml:`name: code-review
version: "1"
...
steps:
  ...
  - id: draft
    ...
    prompt: |
      ...
      Group into: Added, Changed, Fixed, Removed.`}];function Et(){const t=pe[Math.floor(Math.random()*pe.length)];P(`${t.name}  —  ${t.desc}
...
`||i==="."||i===":"?8:3),Y()}else s()}r()})}async function yt(){F("output-dim","gl1tch ready.")}function Be(){for(;$.firstChild;)$.removeChild($.firstChild)}function wt(){if(!k||!E.value.startsWith("/"))return;const t=E.value.slice(1).toLowerCase(),s=[...Object.keys(k.commands),"screenshots","clear","docs"].filter(n=>n.startsWith(t));s.length===1?E.value=`/${s[0]}`:s.length>1&&F("output-dim",s.map(n=>`/${n}`).join("  "))}function Y(){$.scrollTop=$.scrollHeight}function Mt(){const t=document.getElementById("hexbg");if(!t)return;const e=t.getContext("2d"),s=28,n=18;let r,i,c,o,u;function a(){return Math.floor(Math.random()*256).toString(16).padStart(2,"0").toUpperCase()}function l(){t.width=window.innerWidth,t.height=window.innerHeight}function d(){r=Math.ceil(t.width/s)+2,i=Math.ceil(t.height/n)+3,c=Array.from({length:r},()=>Math.random()*i*n),o=Array.from({length:r},()=>.25+Math.random()*.45);const f=i*4;u=Array.from({length:f},()=>Array.from({length:r},a))}function p(){e.clearRect(0,0,t.width,t.height),e.font='12px "JetBrains Mono", monospace',e.fillStyle="rgba(135, 135, 175, 0.07)";for(let f=0;f<r;f++){c[f]=(c[f]+o[f])%(u.length*n);const m=Math.floor(c[f]/n),C=c[f]%n;for(let A=0;A<=i+1;A++){const D=A*n-C,M=(m+A)%u.length;e.fillText(u[M][f],f*s,D)}}requestAnimationFrame(p)}l(),d(),window.addEventListener("resize",()=>{l(),d()}),p()}document.addEventListener("DOMContentLoaded",()=>{Mt(),dt()});
