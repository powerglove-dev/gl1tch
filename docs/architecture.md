...
**CLI subcommands** (pipeline, ask, help, config, etc.) bypass the TUI:
```bash
...
5. BubbleTea initializes Deck Model:
   ...
   - Spawns background goroutines: game engine, narration, telemetry.
...
- `runs` — `id`, `kind`, `name`, `started_at`, `finished_at`, `exit_status`, `stdout`, `stderr`, `metadata`.
- `steps` — `run_id`, `step_id`, `status`, `duration_ms`, `output` (JSON).
- `user_score` — `total_xp`, `level`, `streak_days`, `last_run_date`.
- `game_stats` — aggregates for tuning achievement thresholds.
