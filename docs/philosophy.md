...
**Session.** A named conversation context within the Switchboard. Each session has its own working directory, AI provider, and conversation history. Create sessions with `/session new <name>`, switch with `/session <name>` or `/session <#>`. Sessions let you keep a long-running conversation with one project while running a quick task in another—without losing context in either.
